...
{
	/* Peach */
	mkcol(0, 0xFFFFFFFF, 0x383838FF, 0xC8C8C8FF);
	/* Aqua */
	mkcol(1, 0xFFFFFFFF, 0x383838FF, 0xC8C8C8FF);
	/* Yellow */
	mkcol(2, 0xFFFFFFFF, 0x383838FF, 0xC8C8C8FF);
	/* Green */
	mkcol(3, 0xFFFFFFFF, 0x383838FF, 0xC8C8C8FF);
	/* Blue */
	mkcol(4, 0xFFFFFFFF, 0x383838FF, 0xC8C8C8FF);
	/* Grey */
...
resize(void)
{
	...
	draw(screen, screen->r, allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x383838FF), nil, ZP);
	...
	for(x=startx, i=0; i<nmach; i++,x+=dx){
		...
		string(screen, Pt(x+Labspace, screen->r.min.y + Labspace), display->white, ZP, font, buf);
	}
